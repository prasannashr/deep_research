...
Provider = Literal["openai", "gemini", "openrouter", "groq"]
...
DEFAULT_MODEL_NAME = os.getenv("DEFAULT_MODEL_NAME", DEFAULT_MODELS["openrouter"])
...
def create_model_from_env(variable: str = "DEFAULT_MODEL_NAME"):
    """Create a model from PROVIDER and model environment variables."""
    model_name = os.getenv(variable) or DEFAULT_MODEL_NAME
    provider_name = os.getenv("MODEL_PROVIDER")
    provider = (provider_name or _provider_for_model(model_name)).lower()
